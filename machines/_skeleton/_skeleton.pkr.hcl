...
#
#  Порядок дій:
#    1. cp -r machines/_skeleton machines/<os>
#    2. Перейменуйте _skeleton.pkr.hcl на <os>.pkr.hcl
#    3. Перейменуйте machine.example.json на machine.json
#       і узгодьте в ньому os / name / hostname / box.name
#    4. Покладіть файл автоінсталяції в http/
#    5. Заповніть блок нижче
#    6. packer validate machines/<os>
# ═══════════════════════════════════════════════════════════════

locals {
  # --- Ідентифікація ---
  # os має збігатися з назвою цього каталогу. box.name і hostname шаблон
  # візьме сам з machine.json, що лежить поруч.
